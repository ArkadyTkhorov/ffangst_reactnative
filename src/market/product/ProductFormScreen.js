...
class ProductFormScreen extends Component {
  ...
  static validateForm = fields => (
    ...
    fields.price > 0 &&
    (fields.commType === 0 || fields.phone.length === PhoneField.maxLength) &&
  ...
  componentWillMount() {
    ...
    if (!data.categoryId) data.categoryId = data.categories[0].value;

    data.subcategories = ProductFormScreen.getSubcategories(data.categoryId);

    if (!fields.subcategoryId) fields.subcategoryId = data.subcategories[0].value;

    data.groups = ProductFormScreen.getGroups(fields.subcategoryId);

    if (data.groups.length > 0 && fields.groupId == null) fields.groupId = data.groups[0].value;
  ...
  onFieldChange = (field, value) => {
    ...
    switch (field) {
      ...
      case 'categoryId':
        ...
        fields.subcategoryId = data.subcategories[0].value;

        break;
      case 'subcategoryId':
        data.groups = ProductFormScreen.getGroups(value);
        fields.subcategoryId = value;
        fields.groupId = data.groups.length > 0 ? data.groups[0].value : null;
